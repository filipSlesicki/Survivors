...
[BurstCompile]
public struct CalculateFlowFieldToPortalsJob : IJob
{
	public NativeArray<Cell> grid;
	[ReadOnly]
	public NativeArray<Cell> targets;
	[ReadOnly]
	public int width;
	[ReadOnly]
	public int height;
	...
	{
		ResetCells();
		CreateIntegrationField(targets);
		CreateFlowField();
	}
	/// <summary>
	...
	public void CreateIntegrationField(NativeArray<Cell> destinationCells)
	{
		NativeQueue<Cell> cellsToCheck = new NativeQueue<Cell>(Allocator.Temp);
		for (int i = 0; i < destinationCells.Length; i++)
        {
			Cell target = destinationCells[i];
			target.bestCost = 1;
			target.bestDirectionIndex = direction;
			cellsToCheck.Enqueue(target);
			grid[target.flatIndex] = target;
		}
